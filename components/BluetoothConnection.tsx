...
const BluetoothConnection: React.FC<Props> = ({ onConnect }) => {
  ...
  useEffect(() => {
    // Fix: Access bluetooth property via any cast as it is not in the standard Navigator interface
  ...
  const startScan = async () => {
    // Fix: Access bluetooth property via any cast as it is not in the standard Navigator interface
    if (!(navigator as any).bluetooth) {
      setError("Bluetooth недоступен. Используйте Chrome на Android.");
      return;
    }
          ...
          <div className="space-y-6">
            ...
            {!isBtSupported ? (
              <div className="space-y-4">
                <p className="text-red-500 text-xs font-bold uppercase">Ошибка WebView</p>
                <p className="text-gray-400 text-[10px] leading-relaxed">
                  Android WebView часто блокирует Bluetooth в APK. 
                  Пожалуйста, откройте <span className="text-red-500 font-bold underline">веб-версию сайта в браузере Chrome</span>.
                </p>
              </div>
            ) : (
              <>
